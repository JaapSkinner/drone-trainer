...
def create_settings_panel(parent):
    ...
    theme = create_base_panel_theme()
    # General Settings
    dpg.add_text("General Settings", parent=panel, bullet=True)
    with dpg.group(parent=panel, horizontal=False) as grp:
        dpg.add_text("Username", parent=grp)
        dpg.add_input_text(parent=grp)
        dpg.add_text("Enable Notifications", parent=grp)
        dpg.add_checkbox(parent=grp)
        dpg.add_text("Auto-Save", parent=grp)
        dpg.add_checkbox(parent=grp)
    dpg.add_separator(parent=panel)
    dpg.bind_item_theme(grp, theme)

    # Controls
    dpg.add_text("Controls", parent=panel, bullet=True)
    with dpg.group(parent=panel, horizontal=False):
